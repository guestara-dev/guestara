...
import { X, Search, User, Phone, Mail, FileText, Save } from 'lucide-react'
...
export default function HuespedesPage() {
  ...
  const profileData  = profileName ? (guestProfiles[profileName]??{}) : {}

  return (
    <div className="p-5 space-y-4">
      <div>
        <h1 className="text-xl font-bold">Huéspedes</h1>
        <p className="text-gray-400 text-xs">{guests.length} huéspedes · {reservations.filter(r=>r.status==='checked-in').length} en casa</p>
      </div>

      {/* FIX [A-06] Search */}
      <div className="relative w-64">
        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-500"/>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar huésped..."
          className="w-full bg-gray-800 border border-gray-700 rounded-lg pl-8 pr-3 py-1.5 text-sm focus:outline-none focus:border-violet-500"/>
      </div>
          ...
          return (
            <div key={g.name} className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-gray-700 transition-colors">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-600 to-violet-800 flex items-center justify-center text-sm font-bold shrink-0">
                  {initials}
                </div>
                <div className="flex-1 min-w-0">
                  {/* FIX [A-02] Name is now clickable → opens profile modal */}
                  <button onClick={()=>openProfile(g.name)} className="font-medium hover:text-violet-400 transition-colors text-left truncate w-full">
                    {g.name}
                  </button>
      ...
      {/* FIX [A-02] Guest profile modal */}
      {profileName && profileGuest && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-gray-800">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-600 to-violet-800 flex items-center justify-center font-bold">
            ...
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={editEmail} onChange={e=>setEditEmail(e.target.value)} placeholder="email@ejemplo.com"
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-2.5 py-2 text-sm focus:outline-none focus:border-violet-500"/>
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={editNotes} onChange={e=>setEditNotes(e.target.value)} placeholder="Preferencias, alergias, VIP..."
                  rows={2} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-2.5 py-2 text-sm focus:outline-none focus:border-violet-500 resize-none"/>
              </div>
              <div className="pt-1">
            ...
            <div className="p-4 border-t border-gray-800">
              <button onClick={saveProfile}
                className="w-full flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 py-2.5 rounded-xl text-sm font-medium transition-colors">
